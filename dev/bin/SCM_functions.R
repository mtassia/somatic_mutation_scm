...
#TODO: Error in H5Fopen(file, flags = flags, fapl = fapl, native = native) : 
#TODO: HDF5. File accessibility. Unable to open file.
...
add_scaled_tree_to_h5f <- function(h5f_path, phylo, write = FALSE, overwrite = FALSE, merged = FALSE) {
  ...
  #       Path to h5f file
  #   - phylo:
  #       Phylo object used as input to scm for ass
  #   - write:
...
plot_state_probs_on_tree <- function(locus_str, gt_state_list, tree,
                                     scm_summary, PPthreshold = 0.95,
                                     tip_labels = FALSE) {
  # ARGUMENTS:
  #   - locus_str:
  #       String; name of locus for which SCM was run
  ...
  #       Default = 0.95
  #   - tip_labels:
  #       Boolean; whether to show tip labels on the tree
  #       Default = FALSE

  priors <- gt_state_list[[locus_str]] %>%
    tibble() %>%
    select_if(~ !is.numeric(.) || sum(.) != 0) %>%
    select(!locus) %>% 
    pivot_longer(cols = 2:ncol(.), names_to = "GT", values_to = "prior")

  posteriors <- scm_summary[["gt_posteriors"]][1:(tree$Nnode + 1), ] %>%
    mutate(Indiv = tree$tip.label,
          .before = 1) %>%
    select_if(~ !is.numeric(.) || sum(.) != 0) %>%
    pivot_longer(cols = 2:ncol(.), names_to = "GT", values_to = "posterior")

  prior_post <- left_join(priors, posteriors) %>%
    mutate(across(everything(), ~ replace_na(.x, 0))) %>%
    pivot_longer(cols = 3:4, names_to = "stat", values_to = "prob") %>%
    mutate(stat = factor(stat, levels = c("prior", "posterior")))

  root_node <- length(tr$tip.label) + 1
  germ <- scm_summary[["gt_posteriors"]][root_node, ]
  if (any(germ >= PPthreshold)) {
    germ <- names(which.max(germ))
  } else {
    germ <- NA
