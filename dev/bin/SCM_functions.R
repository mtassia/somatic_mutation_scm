...
multi_scm <- function(gt_state_list, chr_str, h5f_path, tree, Q,
  ...
  if (file.exists(h5f_path) & overwrite == FALSE) {
    cat("h5f file exists and overwrite = FALSE. Checking last entry...\n")
    ...
    # If all last entries are the same, all loci were added correctly.
    # Else, remove last entry and begin run with that locus
    if (length(unique(last_entries)) == 1) {
      ...
      cat("Last entry not added correctly. Removing last entry and restarting...\n")
  ...
  if (file.exists(h5f_path) & overwrite == TRUE & dryrun == FALSE) {
    ...
    #Static groups
    h5write(obj = cat(write.tree(phy = tree)),
            file = h5f_path,
            name = "tree")
    h5write(obj = scm_its,
            file = h5f_path,
