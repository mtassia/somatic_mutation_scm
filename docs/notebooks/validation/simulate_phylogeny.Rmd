---
title: "Simulating hematopoietic phylogeny for SCM validation"
author: "Michael Tassia"
date: "`r Sys.Date()`"
output:
  pdf_document: default
  html_document: default
---

```{r setup, include = FALSE}
knitr::opts_chunk$set(comment = "",
                      fig.align = 'center',
                      out.width = "75%")
```

# Overview

The objective of this notebook is to document our validation strategy for applying stochastic character mapping as a technique for reconstructing somatic genotypes given a phylogenetic tree, substitution model, and a data set of observed genotype states at the tips (see [*Bollback 2006*](https://link.springer.com/article/10.1186/1471-2105-7-88) and [*Revell 2024*](https://peerj.com/articles/16505/)). As a ground-truth somatic phylogeny data set does not yet exist, we will leverage simulation as a framework for validation.

This notebook will first describe the approach, key functions, and expected outcomes. Afterwards, additional layers of technical and biological variation will be introduced, including:

-   Sample size

-   Sequencing depth

-   Missing data

-   Collapsing branch lengths equal to 0

-   ISM-violation rate

# Simulating hematopoiesis

We use the publicly available [`rsimpop` package](https://github.com/nangalialab/rsimpop) developed by Nick Williams at the Sanger Institute to simulate hematopoiesis under a birth-death model. This package implements the Gillepsie algorithm (see [here](https://lewiscoleblog.com/gillespie-algorithm) for basic overview) to simulate cellular compartments under a distribution of user-defined parameters and is described in the supplementary information of [*Mitchell et al. 2022*](https://www.nature.com/articles/s41586-022-04786-y#Sec32).

Here, we use `rsimpop` with parameters set following empirical estimates from the literature. Fitness coefficients manifest as increased symmetric division rates, and polygenic effects are assumed to be additive.

The libraries and functions used in this notebook are tracked in [`simphy_functions.R`](simphy_functions.R).

```{r, warning = FALSE}

suppressMessages(
  source("simphy_functions.R")
)

```

## Distribution of fitness effects (DFE)

`rsimpop` encodes fitness as increased rates of symmetric division (i.e., birth) for clones bearing a driver mutation. Here, we draw fitness effects from a gamma distribution using the parameters described in [Extended Data Fig 12 of *Mitchell et al. 2022*](https://www.nature.com/articles/s41586-022-04786-y/figures/17).

```{r}

# DFE function
fitnessFn <- genGammaFitness(shape = 0.47, rate = 34) 

# Plot DFE
hist(sapply(1:1e4, function(x) exp(fitnessFn()) - 1),
     breaks = 100, xlim = c(0, 1),
     xlab = "Selection coefficient", main = "")

```

## Simulate HSC population

Using the DFE above, we can simulate hematopoiesis with stochastic introduction of driver mutations using the `run_driver_process_sim()` wrapper in `rsimpop`. This function has several important parameters which will be defined below:

-   `simpop = NULL`: Default setting; do not use a pre-existing simpop object

-   `initial_division_rate = 0.1`: Default setting; parameter estimate for HSCs during development; only effects early population growth phase.

-   `final_division_rate = 1/365`: Default setting; parameter estimate for HSCs during adulthood (concordant with the ABC estimates in [*Mitchell et al. 2022*](https://www.nature.com/articles/s41586-022-04786-y)).

-   `target_pop_size = 1e5`: Default setting; mean parameter estimate for HSC population size at adulthood (concordant with the ABC estimates in [*Mitchell et al. 2022*](https://www.nature.com/articles/s41586-022-04786-y)).

-   `nyears = 50`: User-defined age at collecting data. Validation will integrate across a distribution of ages.

-   `fitness = fitnessFn`: User-defined fitness function defined above

-   `drivers_per_cell_per_day = 2.0e-3 / 365`: User-defined count of driver mutations entering the population per day. This is the mean value estimated by ABC in [*Mitchell et al. 2022*](https://www.nature.com/articles/s41586-022-04786-y).

```{r, results = 'hide', warning = FALSE}

# Simulation seed
SEED <- 1212121
initSimPop(SEED, bForce = TRUE)

# Run rsimpop
sim <- run_driver_process_sim(
  simpop                   = NULL,  
  initial_division_rate    = 0.1,  
  final_division_rate      = 1/365,   
  target_pop_size          = 1e5, 
  nyears                   = 80,  
  fitness                  = fitnessFn, 
  drivers_per_cell_per_day = 2.0e-3 / 365  
)

# Sample 100 cells from the HSC population
st <- get_subsampled_tree(sim, 100)

# Re-scale the phylogeny by mutations
st_mut <- get_elapsed_time_tree(st, 
                                mutrateperdivision = 0, 
                                backgroundrate = 16.8/365)

# Plot driver events on the phylogeny
plot_tree_events(st_mut,
                 cex.label = 0,
                 legpos = NULL)

```

Last before creating a mutation matrix that'll ultimately be used to approximate a VCF-like data structure, we need to extract the `phylo` object from the `simpop` output and remove the artificial out group (*s1*).

```{r, warning = FALSE}

# Remove s1 from phylogeny
tr <- get_phylo_object(st_mut) %>% drop.tip("s1")

# Plot phylo
ggtree(tr,
       ladderize = FALSE) +
  layout_dendrogram()

```

To compare this ground-truth tree (where branch lengths are scaled by the true mutation counts) against a molecular phylogeny (e.g., generated by [CellPhy](https://academic.oup.com/mbe/article/39/8/msac147/6649583)), which reports branch lengths in expected substitutions/site, we need to rescale these raw counts by the size of the analyzable genome. Scaling will also account for the reduced genome access, as only genomic positions passing a genome-accessibility filter are the only loci realistically analyzed during phylogenetic inference (we use the [1000 Genomes Project genome-accessibility mask](https://ftp.1000genomes.org/vol1/ftp/release/20130502/supporting/accessible_genome_masks/)).

```{r}

# Path to a downloaded 1000 Genomes accessibility mask (e.g. the "strict" or
# "pilot" whole-genome mask BED/BED.gz from the URL above)
mask_path <- "https://ftp.1000genomes.ebi.ac.uk/vol1/ftp/data_collections/1000_genomes_project/working/20160622_genome_mask_GRCh38/StrictMask/20160622.allChr.mask.bed"

# Ground-truth molecular phylogeny, branch lengths in substitutions/site
tr_mol <- scale_branches_to_subs_per_site(tr, mask = mask_path)

ggtree(tr_mol, ladderize = FALSE) +
  layout_dendrogram()

```

## Simulate mutation data

Because `rsimpop` tracks mutation origin alongside the "ground truth" simulated history, we can use these mutations to reconstruct a VCF-like structure that can later be used for SCM.

```{r}

# Extract phyletic patterns per mutation
mut_mat <- create_mut_df(tr)

# Preview mutation matrix
mut_mat[1:5, 1:5]

```

The `G` object above is a matrix of 0s and 1s, where rows correspond to mutations and columns correspond to sampled cells. A value of 1 indicates that the mutation is present in the cell, while a value of 0 indicates that it is absent. An additional column is included `G[,1]` to track the edge on which the mutation arose -- this is important for tracking accuracy of SCM later.

For example:

```{r}

# Plot tree with edge labels
plot.phylo(tr,
           show.tip.label = TRUE,
           direction = "downwards",
           cex = 0.5)
edgelabels(edge = 10)

# For the first row of G where G[,1] == 10, print columns where mutation is present
ingroup <- mut_mat[mut_mat[,1] == 10,] %>%
  head(n = 1) %>%
  colnames(.)[as.vector(.) == 1] %>%
  colnames()

cat(paste("Mutation events on edge 10 are inherited by the following tips:\n",
          ingroup[1],
          "to",
          ingroup[length(ingroup)]))

```

From this, we can use the `simulate_depth_and_ad` function to simulate sequencing data. Default parameters are set such that they mimic the structure of clean data described in both [*Mitchell et al. 2022*](https://www.nature.com/articles/s41586-022-04786-y) and [Coorens et al. 2025](https://www.nature.com/articles/s41596-024-00962-8).

-   `G`: Matrix output by `create_mut_df()`

-   `D_bar = 20`: Target mean sequencing depth across the whole matrix

-   `min_cov = 6`: Lower depth bound a site must clear to pass QC (Sequoia-style min_cov)

-   `max_cov = 250`: Upper depth bound (excludes repeat/mapping-artifact loci)

-   `site_sdlog = 0.5`: Log-scale spread of per-site coverage bias (GC/mappability)

-   `sample_sdlog = 0.3`: Log-scale spread of per-sample library depth variation

-   `theta = 8`: Negative-binomial dispersion for residual (post-QC) depth noise

-   `rho_max = 0.1`: Beta-binomial overdispersion ceiling a site must clear (Sequoia snv_rho)

-   `site_conc_shape = 4`: Gamma shape for the distribution of per-site VAF concentration

-   `site_conc_rate = 0.3`: Gamma rate for the distribution of per-site VAF concentration

-   `error_rate = 0.002`: Sequencing/mapping error rate (alt reads at true hom-ref sites)

-   `dropout_rate = 0.00`: $P(\textrm{complete allelic dropout} | \textrm{truly heterozygous site})$

```{r}

sim_reads <- simulate_DP_and_AD(G = mut_mat)
glimpse(sim_reads)

```

From the `sim_reads` list output, the `AD` and `DP` data frames contain the simulated read data at each site for each sample. Using these values, we can approximate Phred-scaled genotype likelihoods following a structure similar to the fixed-error-rate GATK model ([DePristo et al. 2011](https://www.nature.com/articles/ng.806)).

```{r}

sim_genotypes <- simulate_GQ_PL_GT(AD = sim_reads$AD,
                                   DP = sim_reads$DP)
glimpse(sim_genotypes)

```

Finally, we will sample sites from the human genome and assign these to the records held in `sim_genotypes`. These sites will then be threaded into a function to create a VCF-like structure.

```{r}

mut_table <- sample_mutation_loci(nrow(sim_genotypes$GT))
vcf_df <- build_vcf_df(G = mut_mat, 
                       DP = sim_reads$DP, 
                       AD = sim_reads$AD, 
                       gl = sim_genotypes,
                       edges = mut_mat$edge)

head(vcf_df[, 1:10])

```

This structure can be exported as a VCF file with `write_vcf_df()`; we will write the ground-truth molecular phylogeny to file alongside it.

```{r}

write_vcf_df(vcf_df, file = "test_sim/test_sim.vcf")
write.tree(tr_mol, file = "test_sim/test_sim.nwk")

```

# Testing the SCM framework on simulated data

Given the simulated tree and somatic variants above, we can now perform SCM and infer the evolutionary history of somatic variants. Here, we'll use the ground truth tree and just test the performance of SCM in reproducing each variant's origin on the tree. In practice, SCM would be performed on a molecular phylogeny inferred from the VCF file above.

The three critical components for modeling somatic SNVs with SCM are (1) the phylogeny with branch lengths measured in substitutions per site, (2) the VCF file with somatic SNVs and their associated confidence metrics (`PL` scores), and (3) a genotype substitution model fit to the phylogeny in (1). Below, we'll use a substitution model fit with `CellPhy` to the ground truth tree.

The `CellPhy` command used to fit model parameters was the following:

```         
cellphy.sh \
    RAXML \
    --evaluate \
    --msa test_sim/test_sim.nwk \
    --msa-format vcf \
    --model GT10+FO \
    --tree test_sim/test_sim.vcf \
    --prefix cellphy \
    --threads 8 \
    --force perf_threads
```

As the function above does produce a phylogeny with branch lengths adjusted for the fit substitution model parameters, a small comparison will be performed below between the ground truth tree and the `bestTree` output by `CellPhy`. The tree output by CellPhy will be used later, as the branch lengths are scaled to the substitution model fit.

```{r}

# read tree
cellphy_tr <- read.tree("test_sim/cellphy.raxml.bestTree")

# Robinson-Foulds distance
TreeDist::RobinsonFoulds(tr_mol, cellphy_tr, normalize = TRUE)

```

In preparation for applying SCM, we'll load all the functions below:

```{r, warning = FALSE}

suppressPackageStartupMessages(
  source("../../../smk/workflow/bin/SCM.smk.R")
)

```

## Validating SCM output

The SCM snakemake pipeline was run on the simulated data above with 100 SCM preplicates per locus. 